// app/(erp)/student/data/[id]/edit/page.jsx
"use client";
...
export default function EditAdmissionPage() {
  ...
  const [isLoading, setIsLoading] = useState(true); // 💡 Start loading as true for Edit page
  ...
  // States for Dynamic Subject Mapping
  ...
  const [formData, setFormData] = useState({
    year: "", yearForId: "", roll: "", classShiftSection: "", className: "", 
    shift: "", section: "", semester: "", term: "", group: "",
    studentId: "", processCode: "", admittedClass: "", studentCategory: "", dateOfAdmission: "", session: "",
    boardRoll: "", registrationNo: "", responsibleTeacher: "",
    ...
    guardianName1: "", guardianName2: "", studentMobile: "", email: "", guardianMobile1: "", guardianMobile2: "",
  ...
  // 1. Initial Dependency & Student Data Fetch
  useEffect(() => {
    const loadAllData = async () => {
      try {
        const [semRes, termRes, grpRes, clsRes, catRes, sesRes, teaRes, occRes, brdRes, presetRes, mappingRes, subMapRes, studentRes] = await Promise.all([
          ...
          api.get("/v1/mappings/class-subject").catch(()=>({data:{data:[]}})),
          api.get(`/v1/students/${id}`).catch(()=>({data:{data:null}})) // 💡 Fetch existing student data
        ]);
        ...
        setClassSubjectMappings(subMapRes.data.data || []);

        // 💡 Populate student data into form
        const student = studentRes.data.data;
        if (student) {
          // Pre-fill null/undefined values with empty strings to avoid uncontrolled input errors
          const sanitizedData = Object.keys(formData).reduce((acc, key) => {
            acc[key] = student[key] !== undefined && student[key] !== null ? student[key] : formData[key];
            return acc;
          }, {});

          // Extract Optional Subject from DB to update the Radio Button UI
          const savedOptional = student.enrolledSubjects?.find(sub => sub.subjectType === "Optional");
          if (savedOptional) setOptionalSubject(savedOptional.subjectName);

          setFormData({ ...sanitizedData, qualifications: student.qualifications || [] });
        }
  ...
  // Dynamic Subject Filtering & Saving Logic
  useEffect(() => {
    if (!formData.className || !formData.semester || !formData.group || classSubjectMappings.length === 0) {
    ...
    if (formData.division) {
      fetch(`https://bdapis.com/api/v1.1/division/${formData.division.toLowerCase()}`)
        ...
        .then(data => {
          ...
          // Do NOT reset upazila here on initial load, otherwise edit form drops the district!
        })
        .catch(console.error);
    } else {
      setBdDistricts([]);
  ...
  const handleChange = (e) => {
    ...
    if (name === "classShiftSection") {
      ...
      // User manual change: reset dependents
      setFormData({ ...formData, division: value, district: "", upazila: "" });
    } else if (name === "district") {
      // User manual change: reset dependent
  ...
  const handleEditSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      // 💡 UPDATED: PUT request to update existing record
      await api.put(`/v1/students/${id}`, formData);
      alert("Student Record Updated Successfully!");
      router.push("/student/data");
    } catch (err) { 
      alert(err.response?.data?.message || "Update process failed"); 
    } finally { 
      setIsSaving(false); 
    }
  };
  ...
  if (isLoading) return <div className="p-10 text-center">Loading Student Data...</div>;

  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-1">Edit Student Record</h1>
      ...
      <form onSubmit={handleEditSubmit} className="bg-white rounded border border-gray-200 p-6 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div><label className={label}>Roll *</label><input type="text" name="roll" value={formData.roll} onChange={handleChange} required className={input} /></div>
            ...
            <div>
              ...
              <select name="semester" value={formData.semester} required onChange={handleChange} className={input}>
                <option value="">-- Please Select --</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className={label}>Term *</label><select name="term" value={formData.term} required onChange={handleChange} className={input}><option value="">-- Please Select --</option>{terms.map(t=><option key={t._id} value={t.nameEnglish}>{t.nameEnglish}</option>)}</select></div>
            <div><label className={label}>Group *</label><select name="group" value={formData.group} required onChange={handleChange} className={input}><option value="COMMON">COMMON</option>{groups.map(g=><option key={g._id} value={g.name}>{g.name}</option>)}</select></div>
            <div><label className={label}>Student ID</label><input type="text" name="studentId" value={formData.studentId} disabled className={`${input} bg-gray-100 cursor-not-allowed`} title="Student ID cannot be changed" /></div>
            <div><label className={label}>Process Code</label><input type="text" name="processCode" value={formData.processCode} onChange={handleChange} className={input} /></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div><label className={label}>Student Category *</label><select name="studentCategory" value={formData.studentCategory} required onChange={handleChange} className={input}><option value="">-- Please Select --</option><option value="General">General</option>{categories.map(cat=><option key={cat._id} value={cat.name}>{cat.name}</option>)}</select></div>
            <div><label className={label}>Date of Admission</label><input type="date" name="dateOfAdmission" value={formData.dateOfAdmission} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Session</label><select name="session" value={formData.session} onChange={handleChange} className={input}><option value="">-- Please Select --</option>{sessions.map(s=><option key={s._id} value={s.name}>{s.name}</option>)}</select></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className={label}>Responsible Teacher</label><select name="responsibleTeacher" value={formData.responsibleTeacher} onChange={handleChange} className={input}><option value="">-- Please Select --</option>{teachers.map(t=><option key={t._id} value={t.name}>{t.name}</option>)}</select></div>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4 border-t">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className={label}>English Name *</label><input type="text" name="name" value={formData.name} required onChange={handleChange} className={input} /></div>
            <div><label className={label}>Bangla Name *</label><input type="text" name="banglaName" value={formData.banglaName} required onChange={handleChange} className={input} /></div>
            <div><label className={label}>Arabic Name</label><input type="text" name="arabicName" value={formData.arabicName} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Short Name</label><input type="text" name="shortName" value={formData.shortName} onChange={handleChange} className={input} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className={label}>Gender *</label><select name="gender" value={formData.gender} required onChange={handleChange} className={input}><option value="">-- Please Select --</option><option value="Male">Male</option><option value="Female">Female</option></select></div>
            <div><label className={label}>Religion *</label><select name="religion" value={formData.religion} required onChange={handleChange} className={input}><option value="">-- Please Select --</option><option value="Islam">Islam</option><option value="Hinduism">Hinduism</option></select></div>
            <div><label className={label}>Father's Name *</label><input type="text" name="fatherName" value={formData.fatherName} required onChange={handleChange} className={input} /></div>
            <div><label className={label}>Bangla Father's Name *</label><input type="text" name="banglaFatherName" value={formData.banglaFatherName} required onChange={handleChange} className={input} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className={label}>Father's National ID</label><input type="text" name="fatherNid" value={formData.fatherNid} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Father's Occupation</label><select name="fatherOccupation" value={formData.fatherOccupation} onChange={handleChange} className={input}><option value="">-- Please Select --</option>{occupations.map(o=><option key={o._id} value={o.name}>{o.name}</option>)}</select></div>
            <div><label className={label}>Mother's Name *</label><input type="text" name="motherName" value={formData.motherName} required onChange={handleChange} className={input} /></div>
            <div><label className={label}>Bangla Mother's Name *</label><input type="text" name="banglaMotherName" value={formData.banglaMotherName} required onChange={handleChange} className={input} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className={label}>Mother's Occupation</label><select name="motherOccupation" value={formData.motherOccupation} onChange={handleChange} className={input}><option value="">-- Please Select --</option>{occupations.map(o=><option key={o._id} value={o.name}>{o.name}</option>)}</select></div>
            <div><label className={label}>Mother's National ID</label><input type="text" name="motherNid" value={formData.motherNid} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Birth Certificate No.</label><input type="text" name="birthCertificateNo" value={formData.birthCertificateNo} onChange={handleChange} className={input} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={label}>Date of Birth</label><input type="date" name="dateOfBirth" value={formData.dateOfBirth} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Blood Group</label><select name="bloodGroup" value={formData.bloodGroup} onChange={handleChange} className={input}><option value="">-- Please Select --</option><option value="A+">A+</option><option value="B+">B+</option><option value="O+">O+</option></select></div>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4 border-t">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className={label}>Guardian Name (1)</label><input type="text" name="guardianName1" value={formData.guardianName1} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Guardian Name (2)</label><input type="text" name="guardianName2" value={formData.guardianName2} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Student Mobile (Optional)</label><input type="text" name="studentMobile" value={formData.studentMobile} onChange={handleChange} className={input} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label className={label}>Guardian Mobile 1 *</label><input type="text" name="guardianMobile1" value={formData.guardianMobile1} required onChange={handleChange} className={input} /></div>
            <div><label className={label}>Guardian Mobile 2 (Optional)</label><input type="text" name="guardianMobile2" value={formData.guardianMobile2} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Father's Mobile (Optional)</label><input type="text" name="fathersMobile" value={formData.fathersMobile} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Mother's Mobile (Optional)</label><input type="text" name="mothersMobile" value={formData.mothersMobile} onChange={handleChange} className={input} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={label}>Relationship 1</label><input type="text" name="relationship1" value={formData.relationship1} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Relationship 2</label><input type="text" name="relationship2" value={formData.relationship2} onChange={handleChange} className={input} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={label}>Present Address</label><textarea name="presentAddress" value={formData.presentAddress} rows="2" onChange={handleChange} className={input}></textarea></div>
            <div><label className={label}>Permanent Address</label><textarea name="permanentAddress" value={formData.permanentAddress} rows="2" onChange={handleChange} className={input}></textarea></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className={label}>Village Name/Short Address (English)</label><input type="text" name="villageEnglish" value={formData.villageEnglish} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Village Name/Short Address (Bangla)</label><input type="text" name="villageBangla" value={formData.villageBangla} onChange={handleChange} className={input} /></div>
            <div><label className={label}>Route</label><input type="text" name="route" value={formData.route} onChange={handleChange} className={input} /></div>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-6 border-t gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={isSaving} className="bg-amber-500 hover:bg-amber-600 text-gray-900 px-8 py-2 rounded-sm font-bold text-sm shadow transition-colors">
