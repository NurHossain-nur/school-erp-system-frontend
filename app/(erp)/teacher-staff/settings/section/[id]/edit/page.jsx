// app/(erp)/teacher-staff/settings/section/[id]/edit/page.jsx
"use client";
...
export default function EditTeacherSectionPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get(`/v1/teachers-presets/sections/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/teachers-presets/sections/${id}`, formData);
      router.push("/teacher-staff/settings/section");
    } catch (error) { 
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Update Teacher Section</h1>
