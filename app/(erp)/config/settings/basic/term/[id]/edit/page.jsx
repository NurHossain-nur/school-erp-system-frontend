// app/(erp)/config/settings/basic/term/[id]/edit/page.jsx
"use client";
...
export default function EditTermPage() {
  ...
  useEffect(() => {
    const fetchTerm = async () => {
      try {
        const res = await api.get(`/v1/terms/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/terms/${id}`, formData);
      router.push("/config/settings/basic/term");
    } catch (error) { alert("Failed to update term"); } 
  ...
  if (isFetching) return <div className="p-10 text-center">Loading term data...</div>;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Update Term</h1>
