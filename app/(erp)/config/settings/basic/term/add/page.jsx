// app/(erp)/config/settings/basic/term/add/page.jsx
"use client";
...
export default function AddTermPage() {
  ...
  const [formData, setFormData] = useState({
    nameEnglish: "", nameBangla: "", orderNo: "", currentTerm: "No", isActive: "Yes"
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/v1/terms", formData);
      router.push("/config/settings/basic/term");
    } catch (error) { alert("Failed to add term"); } 
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Add Term</h1>
        ...
        <Link href="/config/settings/basic/term" className="border border-gray-300 text-gray-600 hover:bg-gray-100 rounded-sm px-4 py-2 text-sm font-medium">BACK TO LIST</Link>
      </div>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div>
              <label className={labelStyle}>Current Term {req}</label>
              <select name="currentTerm" required onChange={handleChange} className={inputStyle} value={formData.currentTerm}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
              <label className={labelStyle}>Is Active {req}</label>
              <select name="isActive" required onChange={handleChange} className={inputStyle} value={formData.isActive}>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <Link href="/config/settings/basic/term" className="bg-[#9fa8b6] hover:bg-gray-500 text-white px-6 py-2 rounded-sm font-medium text-sm">Cancel</Link>
