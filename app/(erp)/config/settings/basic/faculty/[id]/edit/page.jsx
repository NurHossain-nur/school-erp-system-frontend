// app/(erp)/config/settings/basic/faculty/[id]/edit/page.jsx
"use client";
...
export default function EditFacultyPage() {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        const res = await api.get(`/v1/faculties/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/faculties/${id}`, formData);
      router.push("/config/settings/basic/faculty");
    } catch (error) {
  ...
  if (isFetching) return <div className="p-10 text-center">Loading faculty data...</div>;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Update Faculty</h1>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              {/* স্ক্রিনশট ৩ অনুযায়ী লেবেলটি Name* দেওয়া হলো */}
              <label className={labelStyle}>Name{req}</label>
              <input type="text" name="nameEnglish" value={formData.nameEnglish} required onChange={handleChange} className={inputStyle} />
            </div>
            <div>
