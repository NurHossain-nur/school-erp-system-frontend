// app/(erp)/config/settings/basic/student-category/[id]/edit/page.jsx
"use client";
...
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const res = await api.get(`/v1/student-categories/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/student-categories/${id}`, formData);
      router.push("/config/settings/basic/student-category");
    } catch (error) {
  ...
  if (isFetching) return <div className="p-10 text-center">Loading category data...</div>;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Edit Student Category</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input type="text" name="nameBangla" value={formData.nameBangla} required onChange={handleChange} className={inputStyle} />
            </div>
            <div>
