// app/(erp)/config/settings/basic/student-category/add/page.jsx
"use client";
...
export default function AddCategoryPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/v1/student-categories", formData);
      router.push("/config/settings/basic/student-category");
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Add Student Category</h1>
        ...
        <Link href="/config/settings/basic/student-category" className="border border-gray-300 text-gray-600 hover:bg-gray-100 rounded-sm px-4 py-2 text-sm font-medium transition-colors">
          BACK TO LIST
        </Link>
      </div>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label className={labelStyle}>Name Bangla{req}</label>
              <input type="text" name="nameBangla" required onChange={handleChange} className={inputStyle} />
            </div>
            <div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Link href="/config/settings/basic/student-category" className="bg-[#9fa8b6] hover:bg-gray-500 text-white px-6 py-2 rounded-sm font-medium transition-colors text-sm">Cancel</Link>
