// app/(erp)/config/settings/basic/room/[id]/edit/page.jsx
"use client";
...
export default function EditRoomPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/rooms/${id}`, formData);
      router.push("/config/settings/basic/room");
    } catch (error) { alert("Failed to update room"); } 
  ...
  if (isFetching) return <div className="p-10 text-center">Loading room data...</div>;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Update Room Information</h1>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div><label className={labelStyle}>Order No</label><input type="number" name="orderNo" value={formData.orderNo || ''} onChange={handleChange} className={inputStyle} /></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div><label className={labelStyle}>Seat Capacity</label><input type="number" name="seatCapacity" value={formData.seatCapacity || ''} onChange={handleChange} className={inputStyle} /></div>
            <div><label className={labelStyle}>Number Of Column</label><input type="number" name="numberOfColumn" value={formData.numberOfColumn || ''} onChange={handleChange} className={inputStyle} /></div>
            <div><label className={labelStyle}>Number Of Seat Per Column</label><input type="number" name="numberOfSeatPerColumn" value={formData.numberOfSeatPerColumn || ''} onChange={handleChange} className={inputStyle} /></div>
          </div>
