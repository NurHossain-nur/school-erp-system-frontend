// app/(erp)/config/settings/basic/mapping/class-semester/[id]/edit/page.jsx
"use client";
...
export default function EditClassSemesterMappingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [semRes, mappingRes] = await Promise.all([
          api.get("/v1/semesters"),
          api.get(`/v1/mappings/class-semester/${id}`)
        ]);
        
        const mappedData = mappingRes.data.data;
        setSelectedYear(mappedData.year);
        setSelectedClass(mappedData.className);

        const initialState = {};
        semRes.data.data.forEach(s => {
          const existingConfig = mappedData.semesters.find(ms => ms.semesterName === s.nameEnglish);
          if (existingConfig) {
            initialState[s.nameEnglish] = { isSelected: true, orderNo: existingConfig.orderNo || "" };
          } else {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/mappings/class-semester/${id}`, { year: selectedYear, className: selectedClass, semesters: semestersArray });
      router.push("/config/settings/basic/mapping/class-semester");
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mb-4">
             <h2 className="text-lg text-gray-800">Class - <span className="font-bold uppercase">{selectedClass}</span> (Year: {selectedYear})</h2>
          </div>
