// app/(erp)/config/settings/basic/mapping/class-semester/page.jsx
"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { ClassSemesterTable } from "@/components/config/mapping/ClassSemesterTable";
import api from "@/lib/axios";

export default function ClassSemesterMappingPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        ...
        <Link href="/config/settings/basic/mapping/class-semester/add" className="border border-[#434b8c] text-[#434b8c] hover:bg-indigo-50 rounded-sm px-6 py-2 text-sm font-medium">
          ADD NEW
        </Link>
      </div>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden min-h-[300px]">
        {isLoading ? (
          ...
          <ClassSemesterTable data={mappings} refreshData={fetchData} classList={classesList} />
        )}
      </div>
    </div>
  );
