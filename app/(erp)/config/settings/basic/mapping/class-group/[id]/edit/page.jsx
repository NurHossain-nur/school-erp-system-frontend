// app/(erp)/config/settings/basic/mapping/class-group/[id]/edit/page.jsx
"use client";
...
export default function EditClassGroupMappingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [grpRes, mappingRes] = await Promise.all([
          api.get("/v1/groups"),
          api.get(`/v1/mappings/class-group/${id}`)
        ]);
        
        const mappedData = mappingRes.data.data;
        setSelectedClass(mappedData.className);

        // ডাটাবেসের সেভ করা গ্রুপের সাথে মেইন গ্রুপের লিস্ট মার্জ করা
        const initialGroupState = {};
        grpRes.data.data.forEach(g => {
          // চেক করা হচ্ছে এই গ্রুপটি ডাটাবেসে সেভ করা আছে কি না
          const existingConfig = mappedData.groups.find(mg => mg.groupName === g.name);
          
          if (existingConfig) {
            initialGroupState[g.name] = {
              isSelected: true,
              numberOfRequiredSubject: existingConfig.numberOfRequiredSubject || "",
              choosableCompulsorySubjectInstruction: existingConfig.choosableCompulsorySubjectInstruction || "",
              fourthSubjectInstructionForAdmissionForm: existingConfig.fourthSubjectInstructionForAdmissionForm || ""
            };
          } else {
            initialGroupState[g.name] = {
              isSelected: false,
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/mappings/class-group/${id}`, { className: selectedClass, groups: groupsArray });
      router.push("/config/settings/basic/mapping/class-group");
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="w-full md:w-1/3 mb-4">
            ...
            <input type="text" value={selectedClass} disabled className="w-full border border-gray-300 rounded p-2 text-sm bg-gray-50 text-gray-600 font-medium" />
          </div>
