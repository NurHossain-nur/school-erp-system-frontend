// app/(erp)/config/settings/basic/mapping/class-subject/add/page.jsx
"use client";
...
export default function AddClassSubjectMappingPage() {
  ...
  useEffect(() => {
    const fetchDependencies = async () => {
      try {
        const [clsRes, semMapRes, grpRes, subRes] = await Promise.all([
          api.get("/v1/classes"),
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedClass || !selectedSemester) return alert("Please select Class and Semester");
    ...
    setIsLoading(true);
    try {
      await api.post("/v1/mappings/class-subject", { className: selectedClass, semesterName: selectedSemester, subjects: subjectsArray });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-4 border-b border-gray-200">
            <div>
              ...
              <select required value={selectedClass} onChange={(e) => {setSelectedClass(e.target.value); setSelectedSemester("");}} className={inputStyle}>
                <option value="">-- Please Select --</option>
                {classesList.map(c => <option key={c._id} value={c.nameEnglish}>{c.nameEnglish}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-bold text-[#0c2340] mb-2">Select Semester *</label>
              <select required value={selectedSemester} onChange={(e) => setSelectedSemester(e.target.value)} className={inputStyle} disabled={!selectedClass}>
                <option value="">-- Please Select --</option>
                {availableSemesters.map((sem, i) => <option key={i} value={sem}>{sem}</option>)}
              </select>
            </div>
          </div>

          {/* Master Table Header Label */}
          {(selectedClass && selectedSemester) && (
            <h2 className="text-lg text-gray-800 mb-4">Select Subject for Class - <span className="font-bold">{selectedClass}</span> and Semester - <span className="font-bold">{selectedSemester}</span></h2>
          )}
