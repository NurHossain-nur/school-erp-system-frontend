...
import { ClassSectionTable } from "@/components/config/mapping/ClassSectionTable";
...
export default function ClassSectionMappingPage() {
  ...
  //     try {
  //       const res = await api.get("/v1/mappings/class-section");
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        ...
        <Link href="/config/settings/basic/mapping/class-section/add" className="border border-[#434b8c] text-[#434b8c] hover:bg-indigo-50 rounded-sm px-6 py-2 text-sm font-medium">
          ADD NEW
        </Link>
      </div>

      <div className="border border-gray-200 rounded-md overflow-hidden bg-white shadow-sm min-h-[300px]">
        {isLoading ? <div className="flex items-center justify-center h-40 text-gray-500">Loading mappings...</div> : <ClassSectionTable data={mappings} refreshData={fetchMappings} />}
      </div>
    </div>
  );
}