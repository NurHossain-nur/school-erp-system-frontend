// app/(erp)/config/settings/basic/semester/page.jsx
"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { SemesterTable } from "@/components/config/SemesterTable";
...
export default function SemesterManagementPage() {
  ...
  const fetchSemesters = async () => {
    setIsLoading(true);
    try {
      const res = await api.get(`/v1/semesters?search=${searchTerm}`);
      setSemesters(res.data.data);
    } catch (error) { console.error("Failed to fetch semesters"); } 
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Semester List</h1>
        ...
        <Link href="/config/settings/basic/semester/add" className="border border-[#434b8c] text-[#434b8c] hover:bg-indigo-50 rounded-sm px-6 py-2 text-sm font-medium">
          ADD NEW
        </Link>
      </div>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden min-h-[300px]">
        ...
        <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-end">
          <div className="w-full md:w-1/3">
            ...
            <input 
              type="text" placeholder="semester name.." value={searchTerm}
        ...
        {isLoading ? <div className="flex items-center justify-center h-40 text-gray-500">Loading semesters...</div> : <SemesterTable data={semesters} refreshData={fetchSemesters} />}
      </div>
    </div>
  );
}