// app/(erp)/config/settings/basic/route/add/page.jsx
"use client";
...
export default function AddRoutePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/v1/routes", formData);
      router.push("/config/settings/basic/route");
    } catch (error) { alert("Failed to add route"); } 
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#f4f5f8] pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0c2340] mb-2">Add Route</h1>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Link href="/config/settings/basic/route" className="bg-[#9fa8b6] hover:bg-gray-500 text-white px-6 py-2 rounded-sm font-medium text-sm">Cancel</Link>
