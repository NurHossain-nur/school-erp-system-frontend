// app/(erp)/config/settings/general/co-curricular/[id]/edit/page.jsx
"use client";
...
export default function EditCoCurricularPage() {
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        const res = await api.get(`/v1/general-settings/co-curriculer-act/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.put(`/v1/general-settings/co-curriculer-act/${id}`, formData);
      router.push("/config/settings/general/co-curriculer-act");
    } catch (error) { alert("Failed to update activity"); } 
  ...
  if (isFetching) return <div className="p-10 text-center">Loading activity data...</div>;
