// app/(erp)/config/settings/general/moral-behaviors/add/page.jsx
"use client";
...
export default function AddMoralBehaviorPage() {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState({
    name: "", banglaName: "", applicableFor: "General Marksheet", orderNo: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/v1/general-settings/moral-behaviors", formData);
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <Link href="/config/settings/general/moral-behaviors" className="bg-[#9fa8b6] hover:bg-gray-500 text-white px-6 py-2 rounded-sm font-medium text-sm">Cancel</Link>
